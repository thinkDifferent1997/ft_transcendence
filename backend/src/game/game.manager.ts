...
export class GameManager {
  ...
  async createMatch(player: Socket): Promise<GameSession | null> {
    ...
    try {
      const game = new GameSession(roomId, opponent, player);
  ...
  async createTournamentMatch(
    ...
    const game = new GameSession(roomId, player1, player2, tournamentId);
  ...
  async createAIMatch(player: Socket): Promise<GameSession> {
  ...
  private async createAIMatchInternal(player: Socket): Promise<GameSession> {
    const existingGame = this.findGameByPlayer(player);
    if (existingGame) return existingGame;

    const roomId = `room-${Date.now()}`;

    const game = new GameSession(roomId, player, player);
  ...
  submitAnswer(
    ...
    const isPlayer1 = !isAI && player.id === game.player1.id;

    const question = game.questions[game.currentQuestion];

    if (!question) {
      console.warn(`Answer reçu après la fin de la partie (${roomId})`);
      return;
    }
    ...
    if (game.player1ThreeChoice) game.player1ThreeChoice = false;

    if (game.player2ThreeChoice) game.player2ThreeChoice = false;

    if (game.player1HideAnswer) game.player1HideAnswer = false;

    if (game.player2HideAnswer) game.player2HideAnswer = false;
    ...
    if (isPlayer1) game.player1Answered = true;
    else game.player2Answered = true;
    ...
    if (game.player1Answered && game.player2Answered) {
      ...
      if (game.currentQuestion >= game.questions.length) {
        ...
        if (game.player1Score > game.player2Score) winner = 1;
        else if (game.player2Score > game.player1Score) winner = 2;
        else {
          // Draw score = 0
          if (game.player1Time < game.player2Time) winner = 1;
          else if (game.player2Time < game.player1Time) winner = 2;
          else winner = 0;
        }
      ...
      if (isPlayer1) {
        if (hadThreeChoice) game.player1ThreeChoice = false;

        if (hadHideAnswer) game.player1HideAnswer = false;
      } else {
        if (hadThreeChoice) game.player2ThreeChoice = false;

        if (hadHideAnswer) game.player2HideAnswer = false;
      }
      return {
        nextQuestion: true,
        gameOver: false,
  ...
  public buildDisplayedAnswers(
    ...
    if (threeChoice) {
      ...
      displayedAnswers = [
        wrongAnswers[0],
        wrongAnswers[1],
        displayedAnswers.find((answer) => answer.value === question.correct)!,
      ];
    }

    // HideAnswer
    if (hideAnswer) {
      const hiddenIndex = Math.floor(Math.random() * displayedAnswers.length);

      displayedAnswers[hiddenIndex].label = 'Hidden answer';
    }
  ...
  private updateCorrectStreak(streak: number, correct: boolean): number {
    if (correct) {
      if (streak >= 0) return streak + 1;
  ...
  private activateBonuses(game: GameSession) {
    if (game.player1Streak === 3) game.player1ThreeChoice = true;

    if (game.player1Streak === 5) game.player1HideAnswer = true;

    if (game.player1Streak <= -5) game.player1DoublePoint = true;

    if (game.player2Streak === 3) game.player2ThreeChoice = true;

    if (game.player2Streak === 5) game.player2HideAnswer = true;

    if (game.player2Streak <= -5) game.player2DoublePoint = true;
  }

  markPlayerReady(roomId: string, player: Socket | 'AI'): boolean {
    const game = this.games.get(roomId);
    ...
    if (isPlayer1) game.player1Ready = true;
    else game.player2Ready = true;

    if (game.ai) game.player2Ready = true;

    return game.player1Ready && game.player2Ready;
  }

  markQuestionsLoaded(roomId: string, player: Socket | 'AI'): boolean {
    const game = this.games.get(roomId);
    ...
    else game.player2QuestionsLoaded = true;

    // En mode IA, le second joueur est toujours prêt.
    if (game.ai) game.player2QuestionsLoaded = true;

    // Les deux joueurs ne sont pas encore prêts.
    if (!game.player1QuestionsLoaded || !game.player2QuestionsLoaded)
      return false;
  ...
  removeGame(roomId: string): void {
    ...
    if (game?.ai?.timeout) clearTimeout(game.ai.timeout);
