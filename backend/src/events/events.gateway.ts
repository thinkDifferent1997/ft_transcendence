...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleAnswer(
    ...
    @MessageBody()
    data: { roomId: string; answer: string | null; timeLeft: number },
  ) {
    await this.processAnswer(client, data.roomId, data.answer, data.timeLeft);
  }
  ...
  private async processAnswer(
    ...
    if (result.nextQuestion) {
      ...
      if (game.ai) this.startAITurn(game.roomId);
    }
    ...
    if (result.gameOver) {
      if (!game.tournamentId) {
        ...
        const winnerSocket = result.winner === 1 ? game.player1 : game.player2;

        const loserSocket = result.winner === 1 ? game.player2 : game.player1;

        await this.finishTournamentMatch(game, winnerSocket, loserSocket);
        return;
      }
    }
  }
  ...
  private async aiAnswer(roomId: string) {
    ...
    if (success) {
      ...
      answer = wrongAnswers[this.random(0, wrongAnswers.length - 1)];
    }
  ...
  private accelerateAITurn(roomId: string) {
    ...
    if (!game.ai.timeout || !game.ai.answerAt) return;
  ...
  private async recordTournamentMatchAnswers(
    ...
    try {
      const roomParticipants = await this.tournamentService.getRoomParticipants(
        game.roomId,
      );

      const participant1 = roomParticipants.find(
        (p) => p.userId === game.player1Id,
      );

      const participant2 = roomParticipants.find(
        (p) => p.userId === game.player2Id,
      );

      if (!participant1 || !participant2) {
        console.error(
          'Unable to find both room participants for answer recording',
        );
        return;
      }
  ...
  private async finishTournamentMatch(
    ...
    const tournamentResult = await this.tournamentService.reportWinnerFromUser(
      game.roomId,
    ...
    const tournament = this.tournamentState.findTournamentBySemiFinal(
      game.roomId,
    );
    ...
    if (!tournamentResult.tournamentFinished) {
      ...
      const forfeitedUserId = this.tournamentState.consumeFinalForfeit(
        game.tournamentId!,
      );
      ...
      if (forfeitedUserId) {
        ...
        this.emitGameOver(
          loserSocket,
          {
            winner: winnerSocket === game.player1 ? 1 : 2,
          },
          game,
        );
      ...
      if (tournamentResult.readyToStart) {
        const bracket = this.tournamentState.getBracket(game.tournamentId!);

        console.log('Sending bracket:', bracket);

        const participants = await this.tournamentService.getRoomParticipants(
          tournamentResult.nextRoomId,
        );

        const socket1 = this.gameManager.getPlayerSocket(
          participants[0].userId!,
        );

        const socket2 = this.gameManager.getPlayerSocket(
        ...
        const finalGame = await this.gameManager.createTournamentMatch(
          socket1,
          socket2,
          tournamentResult.nextRoomId,
          game.tournamentId!,
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (payload.tfa !== 'authenticated') throw new Error('2FA not completed');

      client.data.userId = payload.sub;
      client.data.username = payload.username;

      this.gameManager.registerPlayer(client.data.userId, client);
  ...
  private async handlePlayerForfeit(client: Socket) {
    ...
    if (game.tournamentId) {
      const winnerSocket =
        game.player1.id === client.id ? game.player2 : game.player1;
  ...
  private async recordOneVsOneMatch(game: GameSession, winner: 0 | 1 | 2) {
  ...
  private async awardGamification(game: GameSession, winner: 0 | 1 | 2) {
    if (!game.player1Id || !game.player2Id) return;

    try {
      const player1CorrectCount = game.questionHistory.filter(
        (q) => q.player1Correct,
      ).length;

      const player2CorrectCount = game.questionHistory.filter(
        (q) => q.player2Correct,
      ).length;
  ...
  handlePlayerReady(
    ...
    const ready = this.gameManager.markPlayerReady(data.roomId, client);
  ...
  handleQuestionsLoaded(
    ...
    const ready = this.gameManager.markQuestionsLoaded(data.roomId, client);
    ...
    if (game?.ai) this.startAITurn(data.roomId);
  }
  ...
  async handleJoinQueue(@ConnectedSocket() client: Socket) {
    ...
    console.log(`${game.player1.id} matched with ${game.player2.id}`);
