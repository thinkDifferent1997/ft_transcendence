...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const [username, setUsername] = useState(""); // Requis par ton backend pour l'inscription
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (onLogin) onLogin(); // Bascule vers le tableau de bord
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-100 via-pink-50 to-cyan-100 flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-white/50">
          ...
          {errorMsg && (
            <div className="mb-6 p-4 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm flex items-center gap-3 animate-in fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Username (visible uniquement à l'inscription) */}
            {isSignUp && (
              <div>
